...
async fn example_multi_turn_conversation() -> Result<()> {
    ...
    client.query("What's the capital of France?".to_string(), None).await?;
    ...
    client.query("What's the population of that city?".to_string(), None).await?;
...
async fn example_with_sessions() -> Result<()> {
    ...
    client.query("Let's do some math problems".to_string(), Some("math".to_string())).await?;
    ...
    println!("\nSession 2 - History:");
    client.query("Tell me about ancient Rome".to_string(), Some("history".to_string())).await?;
    ...
    client.query("What's 15% of 200?".to_string(), Some("math".to_string())).await?;
