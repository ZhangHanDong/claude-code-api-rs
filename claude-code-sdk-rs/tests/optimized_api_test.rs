...
async fn test_oneshot_query() -> Result<()> {
    ...
    let has_assistant_msg = messages.iter().any(|msg| {
        matches!(msg, Message::Assistant { .. })
    });
    assert!(has_assistant_msg, "Should have received an assistant message");
...
async fn test_interactive_mode() -> Result<()> {
    ...
    // Send a message
    client.send_interactive("Hello, can you hear me?".to_string()).await?;
    ...
    // Send follow-up
    client.send_interactive("What's 10 * 10?".to_string()).await?;
    let messages = client.receive_interactive().await?;
...
async fn test_batch_processing() -> Result<()> {
    ...
    println!("Batch processing {} queries took {:?}", queries.len(), elapsed);
