...
impl Neo4jHookCallback {
    ...
    async fn handle_post_tool_use(
        ...
        let input_json = serde_json::to_string(&input.tool_input)
            .unwrap_or_else(|_| "{}".to_string());

        let output_json = serde_json::to_string(&input.tool_response)
            .unwrap_or_else(|_| "{}".to_string());

        let output_truncated = if output_json.len() > self.config.max_output_size {
            format!("{}...[truncated]", &output_json[..self.config.max_output_size])
        } else {
            output_json.clone()
        };
    ...
    async fn execute(
        ...
        match input {
            HookInput::PreToolUse(pre) => {
                self.handle_pre_tool_use(pre, tool_use_id).await
            }
            HookInput::PostToolUse(post) => {
                self.handle_post_tool_use(post, tool_use_id).await
            }
            HookInput::UserPromptSubmit(prompt) => {
                self.handle_user_prompt_submit(prompt).await
            }
            HookInput::Stop(stop) => {
...
mod tests {
    ...
    async fn test_neo4j_hook_callback_integration() {
        ...
        let callback = Neo4jHookCallback::new(
            Arc::new(graph),
            None,
